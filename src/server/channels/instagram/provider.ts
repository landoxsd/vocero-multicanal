...
export class InstagramAdapter implements ChannelProviderAdapter {
  ...
  async sendMessage(
    ...
    const meta = (account.metadata || {}) as { pageAccessToken?: string; instagramId?: string };
    const token = meta.pageAccessToken;

    if (!token) {
      return {
        success: false,
        error: "Falta el Page Access Token para Instagram Direct",
      };
    }
    ...
    try {
      ...
      if (!res.ok || data.error) {
        return {
          success: false,
          error: data.error?.message || "Error al enviar DM de Instagram",
        };
      }

      return {
        success: true,
        externalMessageId: data.message_id || `ig_${Date.now()}`,
      };
    } catch (err: unknown) {
